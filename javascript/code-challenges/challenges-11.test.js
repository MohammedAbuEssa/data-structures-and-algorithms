...
const count = (target, input) => {
  ...
  const numberOfTimes = input.reduce((count, value) => {
    return count + value.filter((value2) => value2 === target).length;
  }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  const filteredArrays = input.map((subarray) => {
    return subarray
...
let findShortest = (data) => {
  // Solution code here...
  const shortest = starWarsData.reduce((shortest, current) => {
    return parseInt(current.height) < parseInt(shortest.height)
      ? current
      : shortest;
  });
